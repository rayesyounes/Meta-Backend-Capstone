...
class MenuTest(CreateMenuItemsMixin, TestCase):
    ...
    def test_created_items(self):
        for idx in self.items.keys():
            ...
            self.assertEqual(Menu.objects.get(pk=idx).__str__(),
                             f"{self.items[idx]['title']}: ${self.items[idx]['price']:.2f}")
...
class BookingTest(CreateBookingsMixin, TestCase):
    ...
    def set_default_booking_parameters(self, idx):
        if self.bookings[idx].get('no_of_guests') is None:
    ...
    def test_created_bookings(self):
        for idx in self.bookings.keys():
            self.set_default_booking_parameters(idx)
